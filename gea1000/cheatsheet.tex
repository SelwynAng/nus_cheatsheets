...
\begin{multicols*}{5}
    ...
    \begin{itemize}
      ...
      \item If we multiply all the values of X and Y by -1, then the correlation coefficient between X and Y will not change
      \item If we remove an outlier from the data set, the correlation coefficient might not change (Eg.  If all the (X,Y) points lie on a negatively sloped straight line including the outlier(s), the r value remains at -1 after any outlier is removed)
      \item If there are only 2 points, the correlation coefficient between X and Y must be 1 or -1
      \item Adding any number to or subtracting any number from X or/and Y will not change the correlation coefficient between X and Y
